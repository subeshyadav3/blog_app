...
//         setSummary(data.summary); // directly set summary
//       } catch (error) {
//         console.error("Error generating summary:", error);
//         alert("Failed to generate summary. Free API Usage Quota Exceeded");
//       } finally {
//         setLoading(false);
//       }
//     };
...
export default function Summarizer({ content }: { content: string }) {
    ...
    const handleClick = async () => {
        ...
        try {
            ...
            const data = await response.json();
            alert("Due to API limitations, summary may not be very accurate.");
    ...
    return (
        <Drawer open={trigger} onOpenChange={setTrigger}>
            <DrawerTrigger
                className="absolute right-5 mt-10 sm:mt-0 z-50 w-10 h-10 rounded-full animate-spin hover:animate-none bg-violet-500 text-white flex items-center justify-center
                before:content-[''] before:absolute before:inset-0 before:rounded-full before:bg-violet-400/50 before:animate-ping before:-z-10"
            >
                AI
            </DrawerTrigger>
            ...
            <DrawerContent className="mx-5 mb-2">
                ...
                <DrawerHeader>
                    <DrawerTitle>Post Summarizer</DrawerTitle>
                    <DrawerDescription>Get a quick summary of the post.</DrawerDescription>
                </DrawerHeader>
                ...
                <DrawerFooter>
                    {!summary && (
                        <Button onClick={handleClick} className={`w-fit mx-auto ${loading ? "animate-pulse" : ""}`}>
